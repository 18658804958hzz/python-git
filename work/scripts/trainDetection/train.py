...
train_type='rubbishDump'
path='D:/python/pythonDemo/cifarTest/tf\work/trash_detection/imgs/tfRecords/'+train_type+'/'
...
# 获取文件列表
print(os.path.join(path, "*.tfrecords"),'kkkkkkkkkkkkkkkkkkk') # D:/python/pythonDemo/cifarTest/tf\work/tf_record2/trashCan/*.tfrecords kkkkkkkkkkkkkkkkkkk
...
with tf.Session() as sess:
    ...
    for i in range(train_steps):
        ...
        for j in range(batch_size):
            ...
            while train_type.encode("utf-8") not in label_list:
            ...
            for index, label in enumerate(rec["object/label"].values):  # 用enumerate建立列表的遍历对象
                # TFRecord中返回bytes类型，需要用str.encode转成bytes用于比较
                if label == train_type.encode("utf-8"):  # 发现目标label
                    # 目标概率
        ...
        if i % 10 == 0:
            ...
            if loss_output < loss_min:
                ...
                fw=open(path+'weight_biases.txt','wb')
